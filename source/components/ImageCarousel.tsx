...
export default class CarouselImage extends React.Component< { [key: string]: any }, { [key: string]: any }> {
    private transformY: number;
    private transformX: number;
    private destinationX: number;
    private destinationY: number;
    private rootNodesCrouselInterval: any;
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            defaultClass: isString(props.defaultClass) ? props.defaultClass : 'CarouselImage',
    ...
    init(delayTime) {
        ...
        if (this.state.inlineStyle && this.rootNode) {
            ...
            for (var x = 0; x <= images.length-1; x++) {
                ...
                inline = this.addAttributeString(inline, 'transition-delay', `${delayTime || ((data.length - x) / 4)}s`);
                    ...
                    >
                        {
                            ...
                            data.map( (o, i) => {
                                ...
                                if (isObject(o) && isObject(o.props)) {
                                    return (
                                        <img 
                                            className='carousel-image'
                                            key={`carousel-image-${i}-${uuid}`}
                                            {...o.props} 
