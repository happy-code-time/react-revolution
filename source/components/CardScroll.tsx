...
class CardScroll extends React.Component< { [key: string]: any }, { [key: string]: any }> {
    private cardsReference:any;
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            defaultClass: isString(props.defaultClass) ? props.defaultClass : 'CardScroll',
    ...
    loadMore(isScrollEvent = false){
        ...
        const clsCardsHolder = `card-group flex ${isMinified ? 'flex-column isMinified' : 'flex-row'}`;
        ...
        for(let x = 0; x <= data.length-1; x++){
            ...
            if(c == itemsPerLine){
                ...
                dataJsx.push(
                    <div key={uuid()} className={`-group ${clsCardsHolder}`}>
                        {
                            singleLines
                        }
                    </div>
        ...
        if(singleLines.length){
            dataJsx.push(
                <div key={uuid()} className={`card-group ${clsCardsHolder}`}>
                    {
                        singleLines
                    }
                </div>
